...
// Styles
const Btnlink = styled(Link)<{ $size: BtnSize, $back?: boolean, $theme: ThemeType }>`
  ${ButtonCommonStyles}
  ${props => props.$back && `
...
const Btn: React.FC<IBtn> = ({ back, name, size = 'small', url, btn, handler }) => {
  const context = useContext<ContextType | null>(ThemeContext)
  if (btn) return <BtnButton onClick={handler}>{name}</BtnButton>
  return <Btnlink to={url!} $size={size} $back={back} $theme={context!.theme}>{name}</Btnlink>
}
