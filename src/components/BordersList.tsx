...
const BordersList = (props: BordersListProps): JSX.Element => {
  const { param } = props;
  ...
  //         `https://restcountries.com/v3.1/alpha?codes=${param}`
  //       );
  ...
  return (
    <Borders>
      ...
      {list.map((el) => (
        <Btn key={el.link} url={`/${el.link}`} name={el.name} />
      ))}
